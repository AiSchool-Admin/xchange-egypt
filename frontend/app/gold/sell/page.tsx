...
const KARAT_OPTIONS = [
  { value: 'K18', label: 'عيار 18', purity: '75%' },
  { value: 'K21', label: 'عيار 21', purity: '87.5%' },
  { value: 'K24', label: 'عيار 24', purity: '99.9%' },
];
...
const CATEGORY_OPTIONS = [
  ...
  { value: 'CHAIN', label: 'سنسال', icon: '⛓️' },
  { value: 'COIN', label: 'عملة ذهبية', icon: '🪙' },
...
export default function SellGoldPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.push('/login?redirect=/gold/sell');
      return;
    }
  ...
  useEffect(() => {
    if (formData.weightGrams && formData.pricePerGram && formData.karat) {
  ...
  }, [formData.weightGrams, formData.pricePerGram, formData.karat]);
  ...
  const getCurrentPrice = () => {
    return prices.find(p => p.karat === formData.karat);
  };
  ...
  const validateStep1 = () => {
    ...
    if (!formData.titleAr.trim()) {
      newErrors.titleAr = 'يرجى إدخال عنوان القطعة';
    }
    if (!formData.karat) {
      newErrors.karat = 'يرجى اختيار العيار';
    }
  ...
  const validateStep2 = () => {
    ...
    if (currentPrice && parseFloat(formData.pricePerGram) > currentPrice.sellPrice * 1.1) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await apiClient.post('/gold/items', {
      ...
      router.push(`/gold/${res.data.data.id}`);
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-yellow-50 to-white" dir="rtl">
      <div className="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-800">بيع ذهبك</h1>
          <p className="text-gray-600 mt-2">
            اعرض قطعتك الذهبية للبيع بأفضل سعر واحصل على المبلغ بأمان
          </p>
        </div>
          ...
          <div className="flex items-center justify-between">
            {[
              ...
              <div key={s.num} className="flex items-center flex-1">
                ...
                {i < 2 && (
                  <div className={`flex-1 h-1 mx-4 rounded ${step > s.num ? 'bg-yellow-500' : 'bg-gray-200'}`} />
                )}
              </div>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="مثال: خاتم ذهب عيار 21 بتصميم كلاسيكي"
              ...
              <div className="grid grid-cols-3 gap-3">
                {KARAT_OPTIONS.map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => setFormData({ ...formData, karat: opt.value })}
                    className={`p-4 rounded-xl border-2 text-center transition-all ${
                      formData.karat === opt.value
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-3">
                ...
                {formData.images.length < 5 && (
                  <label className="aspect-square border-2 border-dashed border-gray-300 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-yellow-500 transition-colors">
            ...
            <button
              ...
              className="w-full bg-gradient-to-l from-yellow-500 to-amber-500 text-white py-4 rounded-xl font-bold text-lg hover:from-yellow-600 hover:to-amber-600 transition-all"
            >
              التالي: التسعير ←
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 space-y-6">
            ...
            {currentPrice && (
              <div className="bg-gradient-to-br from-yellow-50 to-amber-50 rounded-xl p-4 border border-yellow-200">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm text-yellow-700">سعر {KARAT_OPTIONS.find(k => k.value === formData.karat)?.label} اليوم</div>
                    <div className="text-2xl font-bold text-yellow-800">{formatPrice(currentPrice.sellPrice)} ج.م/جرام</div>
                  </div>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={currentPrice ? `السعر المقترح: ${formatPrice(currentPrice.sellPrice * 0.92)}` : 'أدخل السعر'}
                  className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-yellow-500 focus:border-transparent ${
              ...
              <p className="text-xs text-gray-500 mt-2">
                💡 نصيحة: سعر أقل بـ 5-10% من سعر السوق يجذب المشترين بسرعة
              </p>
            </div>
              ...
              <div className="flex gap-2">
                {[0.85, 0.90, 0.95].map((multiplier) => (
                  <button
                    ...
                    className="flex-1 py-2 px-3 bg-gray-100 hover:bg-yellow-100 rounded-lg text-sm font-medium transition-colors"
                  >
                    {Math.round((1 - multiplier) * 100)}% خصم
                    <br />
            ...
            <div className="bg-blue-50 rounded-xl p-4 flex items-start gap-3">
              ...
              <div className="text-sm text-blue-700">
                <strong>عمولة منخفضة:</strong> فقط 0.7% من قيمة البيع. أقل من أي منصة أخرى.
                <br />
                المشتري يدفع 0.7% أيضاً، إجمالي العمولة 1.4% فقط!
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                type="button"
                onClick={handleNextStep}
                className="flex-1 bg-gradient-to-l from-yellow-500 to-amber-500 text-white py-4 rounded-xl font-bold hover:from-yellow-600 hover:to-amber-600 transition-all"
              >
                التالي: المراجعة ←
              </button>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                  <h3 className="text-xl font-bold text-gray-800">{formData.titleAr}</h3>

                  <div className="flex flex-wrap gap-2">
                    <span className="bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-sm">
                      {KARAT_OPTIONS.find(k => k.value === formData.karat)?.label}
                    </span>
                  ...
                  {formData.descriptionAr && (
                    <p className="text-gray-600 text-sm">{formData.descriptionAr}</p>
                  )}
            ...
            <div className="bg-gray-50 rounded-xl p-4 text-sm text-gray-600">
              ...
              <ul className="list-disc list-inside space-y-1">
                ...
                <li>خصم عمولة 0.7% عند إتمام البيع</li>
              </ul>
            </div>
