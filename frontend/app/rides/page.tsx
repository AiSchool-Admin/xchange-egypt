...
import WhatsAppBooking from '@/components/rides/WhatsAppBooking';
...
export default function RidesPage() {
  ...
  const [whatsappBookingOffer, setWhatsappBookingOffer] = useState<RideOffer | null>(null);
  ...
  const handleBook = (offer: RideOffer) => {
    ...
    if (offer.supportsDirectDeepLink) {
      ...
      // For apps without direct deep links: Use WhatsApp booking
      // This opens WhatsApp with a pre-filled booking message
      setWhatsappBookingOffer(offer);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir="rtl">
                  ...
                  {filteredOffers.map((offer) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className={`w-full py-4 ${offer.supportsDirectDeepLink ? offer.providerBgColor : 'bg-green-500'} ${offer.providerBgColor === 'bg-yellow-500' && offer.supportsDirectDeepLink ? 'text-black' : 'text-white'} rounded-xl font-bold text-lg hover:opacity-90 transition-all active:scale-95 shadow-lg flex items-center justify-center gap-2`}
                      >
                      ...
                      {/* Indicator for WhatsApp booking */}
                      {!offer.supportsDirectDeepLink && (
      ...
      {/* WhatsApp Booking Modal - for providers without direct API */}
      {whatsappBookingOffer && pickup && dropoff && (
        <WhatsAppBooking
          provider={{
            id: whatsappBookingOffer.providerId,
            name: whatsappBookingOffer.providerName,
            nameAr: whatsappBookingOffer.providerNameAr,
            emoji: whatsappBookingOffer.providerEmoji,
            bgColor: whatsappBookingOffer.providerBgColor,
          }}
          pickup={{
            name: pickup.name,
          ...
          price={whatsappBookingOffer.price}
          onClose={() => setWhatsappBookingOffer(null)}
        />
      )}
    </div>
  );
